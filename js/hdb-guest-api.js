...
function hydrate(){
 var room=roomNumber();setText(['room-number','menu-room','service-room','bill-room','checkout-room'],room);
 Promise.all([request('experience'),request('rooms')]).then(function(results){
  ...
  if(exp.accent)document.documentElement.style.setProperty('--gold',exp.accent);
